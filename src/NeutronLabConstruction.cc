...
//	ZeroRot->rotateZ(0. *deg);
	...
	G4double core_ID = 0;
	G4double core_OD = 0.001 *CLHEP::mm;
	
	G4double shell_thick = 0.1 * CLHEP::cm;
	G4double shell_ID = 0;
	G4double shell_OD = core_OD + 2*shell_thick;

	G4double shield_thick = 10.0 * CLHEP::cm;
	G4double shield_ID = 0;
	G4double shield_OD = shell_OD + 2*shield_thick;

	G4Orb *core 	= new G4Orb("core", 	core_OD/2);
	G4Orb *shell 	= new G4Orb("shell", 	shell_OD/2);
	G4Orb *shield 	= new G4Orb("shield", 	shield_OD/2);

/*	// screen to detect particles
	G4double screen_thick 	= 0.001 *CLHEP::mm;
	G4double screen_ID 		= 0;
	G4double screen_OD 		= shell_OD + screen_thick*2;
*/
	// screen to detect particles (with shield)
	G4double screen_thick 	= 0.001 *CLHEP::mm;
	G4double screen_ID 		= shell_OD;
	G4double screen_OD 		= shield_OD + screen_thick*2;

	G4Orb *screen 	= new G4Orb("screen", 	screen_OD/2);
	...
	G4PVPlacement *shell_phys 			= new G4PVPlacement(0, G4ThreeVector(0, 0, 0), shell_log, 		"shell_phys", 		shield_log, false, 0);
	...
	G4VisAttributes *core_vis = new G4VisAttributes(blue);
